...
if __name__ == "__main__":
    ...
    sync_bn         = False
    #---------------------------------------------------------------------#
    #   fp16        使用混合精度训练，可减少约一半显存
    #---------------------------------------------------------------------#
    fp16            = True
    #-----------------------------------------------------#
    #   num_classes     建筑物识别：背景(0) + 建筑物(1) = 2类
    #-----------------------------------------------------#
    num_classes     = 2
    #---------------------------------#
    #   所使用的的主干网络：mobilenet
    ...
    #---------------------------------------------------------#
    #   downsample_factor   下采样的倍数，16（显存小用16，显存大用8）
    #---------------------------------------------------------#
    downsample_factor   = 16
    #------------------------------#
    #   输入图片的大小，RTX 3050 4GB建议用320x320或416x416
    #------------------------------#
    input_shape         = [320, 320]
    
    #----------------------------------------------------------------------------------------------------------------------------#
    #   训练参数针对RTX 3050 4GB优化
    #----------------------------------------------------------------------------------------------------------------------------#
    #------------------------------------------------------------------#
    #   冻结阶段训练参数 - 仅微调，节省显存
    #------------------------------------------------------------------#
    ...
    if local_rank == 0:
        ...
        if total_step <= wanted_step:
            ...
            print("\n\033[1;33;44m[Warning] 本次运行的总训练数据量为%d，Unfreeze_batch_size为%d，共训练%d个Epoch，计算出总训练步长为%d。\033[0m"%(num_train, Unfreeze_batch_size, UnFreeze_Epoch, total_step))
